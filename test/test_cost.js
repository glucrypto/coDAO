...
contract("Cost", accounts => {
	...
	it('should allow lessee to pay tax', async () => {
		try{
			await instance.fundTaxAccount(testAsset,{from: newBidder_winner,value:bid3});	
			let tax = await instance.getTaxFund(testAsset);
			let lesseeOut = await instance.getLessee(testAsset);
			console.log("Tax:",tax.toString(10));
			console.log(lesseeOut)
		}
		catch(error){
			assert.equal(error.reason,"You are not the current lessee");
